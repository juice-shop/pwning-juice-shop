...
the link:../part1/challenges.adoc#hacking-instructor[Hacking Instructor] is enabled, as there is a _Coding Challenges_ tutorial attached
to it.
...
link:monitoring.adoc#prometheus-metrics[`juiceshop_cheat_score` metric] but
also sent in each
link:integration.adoc#challenge-solution-webhook[Challenge solution webhook]
call. The `totalCheatScore` value is not persisted across server
restarts, but its calculation is also not irritated by
link:../part1/challenges.adoc#automatic-saving-and-restoring-hacking-progress[automatic]
or
link:../part1/challenges.adoc#manual-progress-and-settings-backup[manual restoring of hacking progress].
...
activities that are
link:../part1/rules.adoc#-things-considered-cheating[definitely considered cheating] while
solving the available hacking challenges:

* >89% after executing all
link:../part3/contribution.adoc#integration-tests[Integration tests] in <1
minute on the author's Windows 10 laptop
* >90% after executing all
link:../part3/codebase.adoc#end-to-end-tests[End-to-end tests] in <20
minutes during GitHub CI/CD on Ubuntu

== Limitations

The cheat scoring assumes that
link:../part1/running.adoc#single-user-restriction[a single user is hacking the Juice Shop]
