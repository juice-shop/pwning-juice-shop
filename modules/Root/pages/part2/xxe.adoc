...
link:security-misconfiguration.adoc#use-a-deprecated-b2b-interface-that-was-not-properly-shut-down[Use a deprecated B2B interface that was not properly shut down].
...
Similar to
link:injection.adoc#let-the-server-sleep-for-some-time[Let the server sleep for some time]
