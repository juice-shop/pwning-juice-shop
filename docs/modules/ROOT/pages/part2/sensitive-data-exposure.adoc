...
This challenge is another one where you need to find some credentials or
a way to authenticate as a specific user without being able to just
exploit a technical vulnerability like SQL Injection.

include::../../partials/hints/iacLeakedKeyChallenge.adoc[]
...
While modern web applications often separate the frontend from the backend via a REST API, developers sometimes forget that the API might return more data than the frontend actually needs or displays. This challenge is about identifying such an endpoint that leaks the password hash of the logged-in user.

include::../../partials/hints/passwordHashLeakChallenge.adoc[]
...
NOTE: If you are running the Juice Shop with a custom theme and product
inventory, the product to inspect will be a different one. The tooltip
on the Score Board will tell you which one to look into.
