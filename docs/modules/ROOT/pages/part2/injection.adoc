...
WARNING: Please note that some NoSQL Injection challenges described below
...
[[_trick_the_chatbot_into_generating_a_coupon_code_for_you]]
== Trick the chatbot into generating a coupon code for you

The support chatbot is equipped with a tool to generate discount coupons for
customers. However, its system prompt contains strict instructions on when
it is allowed to do so. Your task is to bypass these restrictions using
prompt injection techniques.

include::../../partials/hints/chatbotPromptInjectionChallenge.adoc[]

[[_convince_the_chatbot_to_give_you_a_coupon_of_50_or_more]]
== Convince the chatbot to give you a coupon of 50% or more

Now that you know how to trick the chatbot into generating a coupon, let's see
if you can also influence the discount value it provides. The system prompt
specifies a maximum discount of 10%, but perhaps you can "persuade" it to be
much more generous.

include::../../partials/hints/chatbotGreedyInjectionChallenge.adoc[]
...
WARNING: Please note that this challenge is *not available* when running
...
risk of compromise of the underlying system.
