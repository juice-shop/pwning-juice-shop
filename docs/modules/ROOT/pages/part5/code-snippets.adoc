...
The Score Board retrieves the actual code snippet via REST
endpoints `/snippets/<challengeKey>` which returns the actual code snippet plus the
list of vulnerable lines in JSON format (e.g. `+{"snippet":"filterTable
() {\n let queryParam: string = ... }\n }","vulnLines":[6]}+`)

==== Error handling

The following errors can occur when calling the REST endpoint:
