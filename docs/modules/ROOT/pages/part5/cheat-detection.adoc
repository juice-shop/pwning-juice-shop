...
For challenges where the answer is a specific entry in a file exposed by the web application, a source file comparison mechanism is applied. It uses the Sørensen–Dice coefficient to detect high similarity (75% or more) between a user's submission and the exposed source files. This check specifically targets submissions over 100 characters to prevent copy-pasting the entire file as a solution. Detected cheats result in a maximum cheat score being recorded for that challenge solved.
...
The following value for `totalCheatScore` was measured during
a activity that is
xref:part1/rules.adoc#_things_considered_cheating[definitely considered cheating] while
solving the available hacking challenges: It reached **>99.3%** after executing all
xref:part3/codebase.adoc#_end_to_end_tests[End-to-end tests] in **<9
minutes** on the author's Windows 11 laptop.
