...
| `theme`
| Name of the color theme used to render the UI. Options are `bluegrey-lightgreen`, `blue-lightblue`, `deeppurple-amber`, `indigo-pink`, `pink-bluegrey`, `purple-green`, `deeporange-indigo`, `lime-green`, and `neon-fire`. See <<_material_color_themes,Material Color Themes>> for a sample screenshot of each theme.
| `bluegrey-lightgreen`
...
| `model`
| The LLM model identifier used by the chat bot.
| `'gemma4:e4b'`

| `llmApiUrl`
| URL of the OpenAI-compatible API endpoint for the chat bot LLM. Can be pointed at any compatible provider.
| `'http://localhost:11434/v1'`

| `llmMaxRetries`
| Number of connection attempts to the OpenAI-compatible API endpoint before giving up and returning an error.
| `2`

| `sampleQuestions`
| List of questions randomly chosen from when clicking the _Get a suggestion_ button of the chatbot. Each can be the key of a localized string in `frontend/src/assets/i18n/en.json` or custom free-text.
| `- 'CHATBOT_PROMPT_RECOMMENDATION_SUMMER_PARTY' - 'CHATBOT_PROMPT_RECOMMENDATION_POPULAR' - 'CHATBOT_PROMPT_RECOMMENDATION_SUGAR_FREE' - 'CHATBOT_PROMPT_RECOMMENDATION_START_DAY' - 'CHATBOT_PROMPT_RECOMMENDATION_SEASONAL'`
|===

NOTE: Any OpenAI-compatible endpoint can be used by changing `llmApiUrl` (e.g. OpenAI, Azure OpenAI, or self-hosted alternatives). API keys for authenticated endpoints are set via the `LLM_API_KEY` environment variable.
...
| `overwriteUrlForCsrfChallenge`
| URL that should replace the original URL defined in `overwriteUrlForCsrfChallenge` for the xref:part2/broken-access-control.adoc#_change_the_name_of_a_user_by_performing_cross_site_request_forgery_from_another_origin[CSRF] challenge.
...
*Tip:* To make this challenge realistically solvable, include some kind of hint to the blueprint file's name/type in the product image's `Exif` metadata.
| must be defined on exactly one product
| `JuiceShop.stl` on _OWASP Juice Shop Logo (3D-printed)_

| `exifForBlueprintChallenge`
| List of keywords that are supposed to appear as EXIF properties on the image of the xref:part2/sensitive-data-exposure.adoc#_deprive_the_shop_of_earnings_by_downloading_the_blueprint_for_one_of_its_products[Retrieve Blueprint] challenge product. +
*Note:* Will be used in automatic testing for presence of the EXIF metadata but does not have any effect at runtime.
...
| List of keywords which are all mandatory to mention in a feedback or complaint to solve the xref:part2/sensitive-data-exposure.adoc#_identify_an_unsafe_product_that_was_removed_from_the_shop_and_inform_the_shop_which_ingredients_are_dangerous[Leaked Unsafe Product] challenge. Overrides `deletedDate` with `2019-02-1`. +
*Tip:* To make this challenge realistically solvable, provide the keywords on e.g. PasteBin in an obscured way that works well with the "dangerous ingredients of an unsafe product" narrative.
| must be defined on exactly one product
|
...
| `lime-green`
| image:part4/material-theme_lime-green.png[]

| `neon-fire`
| image:part4/material-theme_neon-fire.png[]
|===

== Provided customizations

Several fully re-themed customizations are provided out of the box
by OWASP Juice Shop for demonstration purposes. We also offer an https://github.com/juice-shop/juice-shop-tutorials/blob/master/customization/README.md[18-step customzation tutorial] to create your own. Some out-of-the-box examples provided with OWASP Juice Shop:

image::part4/theme_mozilla.png[Mozilla-CTF theme]

image::part4/theme_oss.png[OpenSecuritySummit Store theme]

Furthermore, the following convenience customizations are provided out-of-the-box
...
image:part4/theme_kn4.png[Photo wall in Kuehne+Nagel theme]

image:part4/theme_kn5.png[Deluxe membership in Kuehne+Nagel theme]

image:part4/theme_kn6.png[Chatbot in Kuehne+Nagel theme]
...
TIP: Since v10.0.0 you can overwrite the
