= Challenge solutions[[_challenge_solutions]]
...
. Open http://localhost:3000/ftp/acquisitions.md to solve the
challenge.
...
. Use developer tools to search for testing credentials in `main.js`.
. Log in with the `testingUsername` and `testingPassword` values.
...
. Register a user to intercept the `/api/Users` request generated by the user registration.
 ** In Firefox, open the Network tab of the developer tools and use "Edit and Resend".
 ** In Burp Suite, use the Proxy - intercept tab. 
. Remove the `email` and `password` values, then send the modified request.
. It will return a 400 Bad Request, but the challenge will still be completed.
...
have been discovered in the past. footnote:5_1_7[https://wiki.owasp.org/index.php/Testing_for_HTTP_Parameter_pollution_(OTG-INPVAL-004)]
____
...
to http://localhost:3000/api/BasketItems/pass:[{id}] in the Network tab of
your DevTools
...
http://localhost:3000/api/BasketItems/pass:[{id}] replacing `+{id}+` with
...
. Install https://metamask.io/download/[Metamask Browser Extension] if not done yet.
...
. Withdraw any amount less than 255 tokens from the faucet until you get a 1000 BEE Balance, the faucet balance https://dreamlab.net/en/blog/post/ethereum-smart-contracts-vulnerabilities-integer-overflow-and-underflow/[underflows] to 255 as soon as the BEE balance becomes less than 0.
...
. Go into the Security Advisory directory at http://localhost:3000/.well-known/csaf
. Then enter the 2017 directory and click the `.sha512` (checksum) file to download it.
. Copy the sha512 checksum into the Customer Feedback section to solve the challenge.
...
https://placecats.com/300/300 and click _Link Image_ while
...
https://listverse.com/2011/07/08/top-20-fruits-you-probably-dont-know/#comment-4541753139[a particular comment from user __Localhorst__]
...
warning on the `Hueteroneel` fruit that "this coupled with Eurogium
Edule was sometimes found fatal"
+
...
cited as a canonical example of weak encryption.footnote:5_1_1[https://en.wikipedia.org/wiki/ROT13]
____
...
http://stylesuxx.github.io/steganography/ gives you gibberish looking something like `+ÿÁÿm¶Û$ÿHÕPü^ÛN'c±UY;fäHÜmÉ#r<v¸+` or
...
http://localhost:3000/#/deluxe-membership?testDecal=https:%2F%2Fplacecats.com%2Fg%2F400%2F500
as this would result in the application to request the logos from
the relative URL
`+assets/public/images/https://placecats.com/g/400/500+` which
...
like
http://localhost:3000/#/deluxe-membership?testDecal=..%2F..%2F..%2F..%2Fredirect%3Fto%3Dhttps:%2F%2Fplacecats.com%2Fg%2F400%2F500%3Fx%3Dhttps:%2F%2Fgithub.com%2Fbkimminich%2Fjuice-shop
...
Shop and how xref:part3/translation.adoc#_helping_with_translations[the translations] are managed.
This will quickly bring you over to
https://crowdin.com/project/owasp-juice-shop which immediately
spoilers __Klingon__ as a supported language. Hovering over the
...
contexts.footnote:5_1_3[http://www.kli.org/about-klingon/klingon-history]
____
...
. Go to link:http:localhost:3000/#/login[pass:[http:localhost:3000/#/login]] and use SQL Injection to log in
with `wurstbrot@juice-sh.op'--` as __Username__ and anything as
__Password__.
...
<<_use_a_deprecated_b2b_interface_that_was_not_properly_shut_down,Use a deprecated B2B interface that was not properly shut down>>
challenge.
. Prepare a https://github.com/dubniczky/Yaml-Bomb[YAML bomb] payload file which expands into a huge file when parsed due to the exponentially used references of variables, for example:
...
the OWASP Juice Shop inventor and author of this ebook.
...
<<_postal_codes_in_germany,postal codes in Germany>>.
...
O-1xxx Berlin.footnote:5_1_4[https://en.wikipedia.org/wiki/List_of_postal_codes_in_Germany]
____
...
online communities.footnote:5_1_5[https://en.wikipedia.org/wiki/Leet]
____
...
. Search on social media (Reddit or BlueSky) to see that a coupon bot posts a new coupon code every month.
. Look for the Juicy Reddit/BlueSky Coupon Bot online to find its GitHub repository https://github.com/juice-shop/juicy-coupon-bot 
. Check the commits by filtering for "API key". You can download the repository on Linux and grep the commits using `git log --grep="API key"`, or check them directly on the GitHub page https://github.com/juice-shop/juicy-coupon-bot/commits/master/
. Find the commit "Introduce secure variable for API key" and view its contents. If you downloaded the repository on Linux, use `git show [commit id]`.
. You'll find the *apiKey: string* in the line that was modified, check the async default export in the previous version.
. Enter the API key string in the Customer Feedback section to complete the challenge.
...
lazily only when (and to the extent) their content is to be used.footnote:5_1_6[https://en.wikipedia.org/wiki/Billion_laughs_attack]
...
. If one sets their username to *admin"); processQuery=null;
...
(user) machines and servers. footnote:5_1_8[https://res.cloudinary.com/snyk/image/upload/v1528192501/zip-slip-vulnerability/technical-whitepaper.pdf]
____
...
+
image::appendix/encryptionkeys_directory.png[Browsable directory encryptionkeys]
...
. Type any URL (e.g. https://placecats.com/100/100) into _Gravatr
URL_ and click _Link Gravatar_. You will realize a request
http://localhost:3000/profile/image/url with the
chosen https://placecats.com/100/100 as parameter `imageUrl`.
. You will find no HTTP request to https://placecats.com/100/100
...
behind the
<<_overwrite_the_legal_information_file,Overwrite the Legal Information file>>
...
. Use the https://remix.ethereum.org/[Remix Online IDE] or the Juice Shop Web3 Sandbox to write your own contract script for exploiting the contract.
. Make sure to use the same metamask wallet as connected on the Juice Shop Web3 Wallet page for the attack.
. We need to write a script contract for a https://scsfg.io/hackers/reentrancy/[Reentrancy attack], the most common and vulnerable form of exploit for contracts.
