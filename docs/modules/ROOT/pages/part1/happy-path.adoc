...
While you can put items into the shopping cart as an anonymous user, you will eventually not be able to submit an order without an account. You first have to log in to the shop with your user
...
Using the "Add to Basket"-button on any item in the products table lets you add that product into your shopping basket. Clicking it multiple times will let you add more items of the same type, unless you reach a purchase-per-user limit or run into a stock shortage. The _Your
...
WARNING: Do not enter any real credit card or address data anywhere in the
...
NOTE: We will cover only a fraction of the available functionality from
...
image::part1/user-menu.png[User Menu with "Orders & Payments" expanded]
image::part1/user-menu2.png[User Menu with "Privacy & Security" expanded]
...
WARNING: Do not enter real address data here!
...
WARNING: Do not enter real credit card data here!
...
NOTE: You can switch to a Web3 wallet with the "Try out our new Crypto Wallet"-link. You need to have one installed in your browser e.g. via MetaMask plugin to do so. This is only needed if you would like to do the Web3 challenges of Juice Shop.
...
You can drill down into a specific order by clicking the truck-icon. Aprt from the products in that order you will now also see its delivery status, expected delivery date, and how many bonus points you earned.

image::part1/track-orders-result.png[Track Orders Result]

NOTE: Just as there was no "real" payment was happening, you will hopefully
understand that there is no "real" order delivery happening - no matter
what the order tracking dialog suggested.
...
// TODO Update screenshot for v20.0.0
...
The _Complaint?_ menu item is shown only to logged in users. It brings
...
In the _AI Chat_ you can talk to an LLM-powered chatbot
...
TIP: If you want to know more about (or even help with) the localization of
