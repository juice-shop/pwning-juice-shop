...
IMPORTANT: Juice Shop requires an external AI/LLM provider for its chatbot feature. See <<_aillm_provider>> for setup instructions.
...
TIP: As a little related anecdote, the OWASP Juice Shop was crowned
...
and once more
https://hello.heroku.com/webmail/36622/844098776/9fe33b8eda9eb79bca7ee569888b1874[in March 2019]:
...
TIP: A step-by-step guide can be found in https://alphasec.io/practice-hacking-skills-with-owasp-juice-shop/[this blog post].
...
NOTE: Please be aware that support by the Project Leaders or community is
...
Juice Shop's _AI Chat_ requires an external LLM provider to function. By default, it connects to a local https://ollama.com[Ollama] instance at `http://localhost:11434/v1`.

To set up Ollama:

. Install Ollama either via the https://ollama.com[Desktop App] or via the https://ollama.com/download[CLI installation instructions].
. Pull the default model either via the Desktop App's model search or using the CLI:
+
----
ollama pull qwen3.5:9b
----
. Start Ollama (if not already running) and then launch Juice Shop as usual.

NOTE: `qwen3.5:9b` is the recommended default model. It is the smallest model we found to work reliably enough to be useful, and should still run on most machines, even if slowly.

You can configure an alternative OpenAI-compatible endpoint and model via the xref:part4/customization.adoc#_chatbot_subsection[chatBot customization properties] `llmApiUrl` and `model`. For authenticated endpoints, set the `LLM_API_KEY` environment variable with your API key.

NOTE: For the author the `stepfun/step-3.5-flash:free` model on `https://openrouter.ai/api/v1`. worked very well. It responded very fast even in tool-call-heavy conversations. After signing up, you can https://openrouter.ai/workspaces/default/keys[create an API key in the OpenRouter dashboard].

=== Alchemy Ethereum Testnet

Two of Juice Shop's Web3 challenges require a connection to an Ethereum testnet via the https://alchemy.com[Alchemy] API. In order to reliably solve these challenges, you need to set up an Alchemy account and configure the API key in your Juice Shop instance:

. Sign up for an account on https://alchemy.com/ and log in to your dashboard.
. Create an app in the Alchemy dashboard or use the one that is created by default.
. In the _Endpoints_ section, enable **Ethereum Sepolia** (`https://eth-sepolia.g.alchemy.com`) as a network.
. Copy your _API Key_ and set it as an environment variable `ALCHEMY_API_KEY` for your Juice Shop instance
