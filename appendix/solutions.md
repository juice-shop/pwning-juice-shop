...
```
4. Verify that the username got changed to "CSRF" by checking the user profile.

In an actual attack scenario, the attacker will try to trick a legitimate user into opening an attacker-controlled
website. If the victim is simultaneously logged into the website, the requested that is generated by the malicious
