...
3. In the upper part of the page, paste the HTML following code, which contains a self-submitting HTML form:
```html
...
```
4. The attack is performed immediately. The browser might display an error message, but this does not necessarily mean
   that the attack was not successful (the other origin is allowed to send requests to Juice Shop, but not to read the response).
5. Verify that the username got changed to "CSRF" by checking the user profile.
...
form in step 3 is authenticated with the victim's session. The attacker will also hide the automatically
