...
| Custom script sending 10 sequential `POST` requests to [Feedback API endpoint](http://localhost:3000/api/Feedbacks) within 10 seconds                                                   | Must pin `captchaId` and `captcha` from one initially received and solved CAPTCHA                             | 🧠🧠              | ⏳                 | CAPTCHA Bypass(✔️)                                                                                                                                     |
| Custom script sending 3 simultaneous `POST` requests to [Like Reviews API endpoint](http://localhost:3000/rest/products/reviews)                                                        | Requests must be processed within 150ms time window to exploit race condition flaw                            | 🧠🧠🧠           | ⏳                 | Multiple Likes(✔️)                                                                                                                                     |
| 🛠️ **TODO**                                                                                                                                                                          |                                                                                                               |                    |                   |                                                                                                                                                          |
